...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        calculateSums(leftSums, 0, n / 2, 0);
        calculateSums(rightSums, n / 2, n, 0);
    ...
    static void calculateSums(ArrayList<Integer> sums, int start, int end, int sum) {
        if (sum > maxWeight) return;
        if (start == end) {
            sums.add(sum);
            return;
        }
        calculateSums(sums, start + 1, end, sum);
        calculateSums(sums, start + 1, end, sum + items[start]);
    }
